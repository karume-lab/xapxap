...
export function CreatePostScreen() {
  ...
  // Fix: Only pass a valid URI to useVideoPlayer — empty string means no video selected yet.
  // Guard player.pause() so it only fires when a video is actually loaded.
  const videoUri = media?.type === "video" ? media.uri : null;

  const videoPlayer = useVideoPlayer(videoUri, (player) => {
    player.loop = true;
    // Only pause if a real video source is provided
    if (videoUri) {
      try {
        player.pause();
      } catch {
        // Ignore if player not yet ready
      }
    }
  });
  ...
  const pickDocument = async () => {
    try {
      ...
      showDialog("Error", "Could not pick document.");
    }
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      router.replace({ pathname: "/post/[id]", params: { id: newPost.id } } as any);
    } catch {
      showDialog("Could not drop wave", "Something went wrong.");
    } finally {
      setBusy(false);
    }
  };
